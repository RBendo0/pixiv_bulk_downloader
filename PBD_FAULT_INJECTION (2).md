...
## 10. Campione limitato e rotazione delle opere

Il fault injection non dovrà generare continuamente nuove interrogazioni
verso Pixiv.

Durante la preparazione del test verrà recuperato online un **campione
limitato di opere reali**. Una volta ottenuti i dati necessari, il
simulatore riutilizzerà ciclicamente quelle stesse opere per alimentare
le successive iterazioni del test.

Concettualmente:

``` text
recupero online di un campione limitato
    ↓
dati e metadata reali
    ↓
rotazione ripetuta dello stesso campione
    ↓
simulazione download / conversioni
    ↓
fault injection
```

Questo serve a mantenere separati due fenomeni differenti:

-   gli errori deliberatamente prodotti dal sistema di fault injection;
-   gli errori reali derivanti dall'interazione con Pixiv, in
    particolare i rate limit.

Senza questa separazione, un test sufficientemente lungo o aggressivo
potrebbe provocare un vero rate limit dell'API e mescolarlo ai fault che
stiamo cercando di generare e osservare artificialmente.

La rotazione di un campione già acquisito permette invece di sottoporre
ripetutamente il codice interno agli stessi dati, senza aumentare
proporzionalmente il traffico verso Pixiv.

Un eventuale test specifico della gestione dei rate limit dovrà quindi
poter utilizzare **rate limit iniettati deliberatamente**, distinguibili
da quelli autentici prodotti dal servizio remoto.

La quantità esatta di opere da acquisire per il campione non è ancora
definita e verrà scelta durante l'implementazione.
...
## 13. Reinizializzazione del percorso `bookmarks`
...
## 14. Visibilità del percorso durante l'inizializzazione
...
## 16. Sequenza prevista di implementazione
...
## 17. Decisioni ancora da prendere
