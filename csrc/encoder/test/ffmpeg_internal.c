...
int ffmpeg_decode_image(EncoderContext *ctx, const void *data, size_t size) {
  ...
  */
  ...
  if (ctx->frame_count == 0) {
    ...
    /*
    ---------------------------------------------------------
        Create the pixel format conversion context.

        The source pixel format is normalized before conversion.
        JPEG full-range information is configured separately.
        Future revisions may force a conversion to
        a codec-supported format.
    ---------------------------------------------------------
    */

    enum AVPixelFormat source_pixel_format = ctx->pixel_format;
    ...
    ctx->sws =
        sws_getContext(ctx->width, ctx->height, source_pixel_format,

                       ctx->width, ctx->height, ctx->video_encoder->pix_fmt,
  ...
  -------------------------------------------------------------
      ...
      The frame must be writable before copying new pixels.
  -------------------------------------------------------------
  */
  ...
  /*
  -------------------------------------------------------------
      Convert (or simply copy) the image.

      libswscale performs both pixel format conversion and image
      scaling if required.
  -------------------------------------------------------------
  */
